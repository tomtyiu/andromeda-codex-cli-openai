...
function activate(context) {
    ...
    const fixCodeDisposable = vscode.commands.registerCommand('codex.fixCode', async () => {
        ...
        await (0, diffUtil_1.showDiffInEditor)(original, modified, 'Codex Fix Diff');
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codex.fixCode', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
...
async function getCodexFix(original) {
    // Write the original code to a temp file
    ...
    const fixed = await new Promise((resolve, reject) => {
